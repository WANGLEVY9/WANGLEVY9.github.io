...
*.proto linguist-language=Protocol Buffers

[merge "keepours"]
    name = keep ours for generated files
    driver = true
```
- 对生成文件（锁文件、代码生成物）按团队约定选择 `merge=union` 或自定义驱动，减少伪冲突。
- 对 CRLF/LF 混杂的仓库指定 `eol=lf`，配合 `.editorconfig` 减少空白差异。
- 大仓库可按目录拆分 owner 与策略，如 `docs/** merge=union`、`infra/** merge=ours`（谨慎使用）。

## 23. 合并策略对比表
- **merge --no-ff**：保留合并节点，适合长期分支、需要审计的场景。
- **merge --squash**：把分支多个提交压成一个，不产生合并节点；适合短期功能分支。
- **rebase**：线性历史，便于 bisect；不保留分支点。
- **rebase --rebase-merges**：保留分支结构（以 rebase 方式重放），适合在保持拓扑同时消除多余合并噪音。
- **octopus merge**：一次合并多个分支，适合无冲突的独立特性；若有冲突会失败。

## 24. 语义冲突检测的自动化思路
- **契约测试**：对接口/事件/协议提供 consumer/provider 测试，合并后自动跑，捕获行为冲突。
- **Golden file**：关键算法输出对比，合并后比对黄金样本，发现隐性行为差异。
- **变更影响分析**：路径触发对应测试套件；结合 coverage 识别未覆盖路径，提示补测。
- **日志/指标断言**：在 CI 冒烟脚本中加入日志片段/指标阈值断言，合并后快速暴露语义偏差。

## 25. 大规模重排/格式化的安全策略
- 将「全仓格式化/重排」与逻辑改动分离，独立 PR；合并后立即同步主干，减少后续冲突。
- 在 PR 描述中写明「无逻辑改动」并附自动化验证（如快照对比、回归测试全绿）。
- 对仍在活跃开发的目录，先征求 owner 同意；必要时在冻结窗口执行。

## 26. 子模块与多仓场景
- 冲突优先解决子模块仓库：确保子模块指针指向可构建的提交，然后回父仓库更新指针。
- 对子模块同时移动/重命名目录，先在子模块完成并推送，再在父仓库更新路径引用，减少双向冲突。
- 建议为子模块加保护：禁止指向未发布的临时分支；在 CI 检查子模块指针是否匹配发布策略。

## 27. 合并演练剧本（可用作团队练习）
- **剧本 1：删除 vs 修改**：一人删除函数，另一人修改其内部逻辑。目标：合成保留还是删？写出验证脚本。
- **剧本 2：目录重命名 + 新功能**：一人改名并格式化，另一人添加新函数。目标：使用 `-X rename-threshold` 或手动移动后再合并。
- **剧本 3：锁文件冲突**：模拟 npm 依赖升级与 pnpm 依赖升级交叉，验证 `.gitattributes` 与自动合并策略。
- **剧本 4：二进制冲突**：两个分支都更新同一图片。目标：选择来源并写明版权与来源，避免随意覆盖。

## 28. 工具与命令脚本化
- 预检冲突脚本（在 CI 或本地临时分支运行）：
```bash
#!/usr/bin/env bash
set -euo pipefail
...
git fetch origin
git switch -c "$TMP" "$TARGET"
git merge --no-commit --no-ff "$GITHUB_HEAD_REF" || true
...
git branch -D "$TMP"
```
- rerere 状态检查：`git rerere status`；清理缓存：`git rerere clear`；适合定期在大分支上执行。
...
- **指标定义**：
  ...
  - 解决时长 = 冲突出现到合并完成的时间；
  - 返工率 = 解决冲突后引入缺陷的比例；
  - 热点文件 = 一段时间内冲突次数最高的文件列表。
- **可视化**：
  - 冲突热力图按目录/文件；
  - 解决时长箱线图，定位超长案例；
  - rerere 命中率曲线，评估自动化收益。
- **改进行动**：对热点文件设 owner、拆模块、加入契约测试；对超长案例做复盘，沉淀剧本。

## 30. FAQ（扩展版）
- 需要总是 rebase 吗？不。团队约定最重要：私人分支可 rebase，公共分支合并前保持一致即可；禁止对公共分支强推历史。
- 冲突解决后 CI 还需要全跑吗？视风险分级。最少跑改动路径相关测试；高风险或公共模块改动建议全量或夜间回归。
- rerere 会不会用错？若冲突上下文变化大可能误用，解决后检查 diff；对关键文件可暂时禁用 rerere 或手动确认。
- 目录重命名后历史会丢吗？默认 `git log --follow` 可追溯；若重命名+大改动，建议先纯重命名提交，保留历史清晰。

## 31. 结语（扩展）
合并冲突是复杂协作的常态。提前规划（owner、路径触发、冻结窗口）、过程工具化（预检、rerere、契约测试）、事后度量（冲突率、返工率），能把冲突从「事故」变成「可控成本」。让每次冲突解决都有剧本、有验证、有留痕，团队才能稳态迭代。
