...
## 25. 复杂场景剧本补充
- **场景 F：多人同时在共享分支误操作**
  - 处理：先 `git fetch` 同步远端；在新分支用 `git log --graph` 找分叉点；对需要保留的提交逐个 `cherry-pick`；完成后开 PR 让团队评审。
- **场景 G：错误 rebase 覆盖了远端提交**
  - 处理：在 reflog 中找到 rebase 开始前的哈希；`git switch -c restore/<ts> <hash>`；与远端分支做 `git range-diff`，确认遗漏的提交再 cherry-pick 回来；禁止直接强推，走 PR 合并。
- **场景 H：误删标签或发布点**
  - 处理：在 `git reflog --date=iso` 中查找标签创建/删除记录；`git tag <name> <hash>` 恢复；在发布记录中补充说明，避免版本错乱。
- **场景 I：二进制/大文件被误删**
  - 处理：在 `git log -- path` 找到最新提交；`git checkout <hash> -- path` 恢复；若历史过大，考虑移入制品仓，避免再次误删。

## 26. 自动化与防呆
- **危险命令保护**：
  - shell alias 加确认：`alias grh='read -p "Hard reset? (y/N) " ans && [[ $ans == y ]] && git reset --hard'`。
  - pre-push 钩子检查分支：阻止直接向 `main` 强推；提示是否为正确分支。
- **回退脚本模板**：
```bash
#!/usr/bin/env bash
set -euo pipefail
...
TARGET=${1:-""}
if [ -z "$TARGET" ]; then
  echo "Usage: rollback.sh <commit-or-tag>"; exit 1; fi

echo "Rolling back to $TARGET" 
git revert --no-edit "$TARGET"..HEAD || { echo "Revert failed"; exit 1; }
git push origin HEAD
./scripts/smoke.sh
```
- **备份提醒**：在 CI 或 shell 登录时输出提醒（motd），提示距离上次 `git tag temp/backup-yyyymmdd` 已超过 7 天。

## 27. 数据安全与审计留痕
- 对生产回退：
  - 记录时间线、命令、观察到的指标变化；
  - 将回滚提交与变更单/工单关联；
  - 在运行手册中补充「触发原因、决策过程、后续动作」。
- 对安全事件（如敏感信息泄露）：
  - 在私有分支完成历史重写；
  - 使用 `git filter-repo` 或 `BFG` 清理；
  - 重新生成凭证并记录轮转；
  - 强制所有贡献者拉取干净历史，留痕在公告与变更单中。

## 28. 备份与灾备策略
- **本地快照**：在大改前打轻量 tag 或 stash；对重要分支建镜像备份分支 `backup/<name>` 推到远端。
- **异地备份**：关键仓库每日镜像到只读远端；对安全要求高的仓库启用签名与审计。
- **制品与数据对齐**：回退代码时同步回退制品/配置/数据迁移；把回滚脚本与版本号绑定。
- **恢复演练**：定期演练「远端宕机、从镜像恢复」与「敏感信息泄露后的历史重写」，记录耗时与缺口。

## 29. 指标与健康度
- **操作安全指标**：
  - 强推次数、回滚次数、误操作恢复耗时；
  - stash 遗留时间（长期 stash >14 天数量）；
  - 恢复成功率（恢复到正确状态的比例）。
- **可观测性**：
  - 回退后 30/60 分钟关键指标（错误率、延迟）对比；
  - 变更事件与告警关联，确保回退动作可追踪。
- **改进行动**：当强推或误操作频繁时，增加保护（branch protection、pre-push check）、加强培训或调整流程。

## 30. FAQ（扩展版）
- revert 后发现回滚也有问题怎么办？可以对回滚提交再次 revert（revert of revert），或在新提交修复；发布前务必重新验证。
- stash 太多怎么看？定期 `git stash list` 清理；为长期任务开分支，不要把工作压在 stash。
- 如何避免把临时文件提交？完善 `.gitignore`，在 pre-commit 里添加禁止提交临时/大文件的规则。
- 回滚后发现数据库已变更怎么办？如果不可逆，记录影响范围，执行补救脚本；未来在代码与数据层都提供回滚路径。

## 31. 结语（扩展）
回退与救援的终点是「可恢复性」：出现问题时，团队有剧本、有工具、有审计、有训练。把防呆、备份、演练纳入日常，真正做到「犯错可以快速、安全地修复」，让开发勇于尝试又不至于破坏生产。
